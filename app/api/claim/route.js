...
export async function POST(req){
  try{
    ...
    const ip=req.headers.get("x-forwarded-for")||""; const ua=req.headers.get("user-agent")||"";
    const ipHash=crypto.createHash("sha256").update(ip).digest("hex").slice(0,32);

    const supa=supaAdmin();
    const { data, error } = await supa.rpc("claim_code",{
      p_code:code.trim(), p_wallet_address:wallet.trim(), p_tier:"alpha", p_ip_hash:ipHash, p_user_agent:ua
    });
    if(error) return NextResponse.json({error:error.message},{status:400});

    return NextResponse.json({ok:true}); // add Discord later if you want
