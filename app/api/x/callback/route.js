...
export async function GET(req) {
  ...
  if (!state || !authCode) return bounceError(req, state, "missing_state_or_code");

  const supa = supaAdmin();

  // 1) Load PKCE verifier + purpose
  ...
  try {
    ...
    if (process.env.TW_CLIENT_SECRET) body.set("client_secret", process.env.TW_CLIENT_SECRET); // recommended for web apps
    ...
    if (!resp.ok) {
      const reason = tokenJson?.error_description || tokenJson?.error || `token_http_${resp.status}`;
  ...
  if (!accessToken) return bounceError(req, state, "no_access_token");
  ...
  if (!user?.id || !user?.username) return bounceError(req, state, "invalid_profile");

  // 4) Link vs sign-in
  if (stateRow.purpose === "link") {
    if (!stateRow.claim_id) return bounceError(req, state, "missing_claim_for_link");
    ...
    if (updErr) return bounceError(req, state, `claim_update_failed:${updErr.message}`);
  } else if (stateRow.purpose !== "signin") {
    return bounceError(req, state, "invalid_purpose");
  }

  // Optional: consume the state
